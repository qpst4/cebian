...
| 快速启动器 | 扩展 → 快速启动器 | 侧滑网格启动器，多面板与背景不透明度 |
...
├── shell_icons/               # Shell 命令自定义图标（若有）
├── shortcut_icons/            # Activity 快捷方式自定义图标（若有）
...
| 搜索面板历史 | 是 | `search_panel_history.json` 内容 |
...
- 默认导出全部应用设置、`search_icons`、`shell_icons`、`shortcut_icons`、`float_ball_assets`、`stash`，以及上表「默认导出」的嵌入 JSON
...
| `:core:native-engine` | 内置 OCR/翻译/分词引擎包管理与解压 |
