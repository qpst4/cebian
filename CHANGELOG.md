...
### Changed
- **手势**：触钮组合手势（内滑后转向的 L 形、内滑后回缩的折返）不再要求「短滑后悬停」：第一段以向内方向越过短滑距离即就绪，第二段以沿边为主转向（或回缩）即可触发；未配置组合槽位时方向判定照旧（内滑后上/下仍走斜上/斜下，转得够大仍走上下滑）。折返回缩门槛 16dp → 28dp，避免免悬停后末尾微回弹误触；「短滑后悬停」槽位与悬停时长滑条保持不变
- **手势**：折返手势新增撤销区——手指回到起手位置附近（含一路滑到屏幕边缘）抬手即取消，不再强制判折返；「短滑后悬停」时长默认 250 → 500 毫秒、范围 150–500 → 0–1000 毫秒并改用 MIUIX Custom Key Points（0 毫秒＝越过短滑距离即算悬停），悬停静止容差 12dp → 6dp，慢滑的减速停顿不再算作悬停
- **清理**：删除早已失效的「向内复合模式」配置（`inward_hover_compound_enabled`）——识别器与设置页均不再引用它
- **本地化**：补齐应用选择页新增三条文案（更多 / 显示系统应用 / 隐藏系统应用）的日语与阿语翻译；删除 0 条字符串的空壳 `values-arz`（未在 `locales_config` 注册、无任何引用）

### Fixed
- **手势**：设置组合手势的短滑档不再连带写入配对的长滑档，两档完全独立；侧栏手势槽位子页返回后停留原来的 tab（此前总是回到「短距」）
- **手势**：修复「短滑后悬停」在持续滑动中被误判成立——悬停计时改为以**固定锚点**计算，手指离开锚点 6dp 即锚点前移并重新计时，滑动途中不会再攒够时长（此前锚点跟随手指 + 单帧 8dp 才清零，慢速滑动会边滑边攒计时）
