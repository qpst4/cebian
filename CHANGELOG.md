...
### Added
- **快速启动器**：新增面板“模糊强度”设置（0–150 dp），可调节快速启动器面板与文件夹的毛玻璃模糊半径；默认外观调整为背景不透明度 63%、模糊强度 16 dp
- **小组件面板**：默认外观调整为模糊强度 19 dp、面板不透明度 25%
- **快捷方式列表统一**：快速启动器/蜂窝启动器编辑器的“快捷方式”页顶部新增“我的直达”与“预设快捷方式库”文件夹入口；“应用内直达 → 添加应用快捷方式”列表顶部新增“预设快捷方式库”文件夹入口（保留系统快捷方式目录），进出文件夹带滑动过渡

### Fixed
- **小组件面板设置**：修复模糊强度调整后设置页仍回跳到默认值的问题（设置页投影漏传 `widgetPanelBlurRadiusDp`）
- **快捷方式编辑器**：修复从“我的直达/预设快捷方式库”返回后 tab 跳到“动作”的问题（tab 状态改为屏幕级保留）
- **快捷方式列表样式**：修复“我的直达/预设快捷方式库”文件夹卡片左右贴边且未跟随 miuix 组件与配色的问题（改用 `groupedCardItems` + `ArrowPreference`）
