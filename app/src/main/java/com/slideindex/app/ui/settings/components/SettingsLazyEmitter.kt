...
class SettingsLazyEmitter internal constructor() {
    ...
    internal fun registerGroupedCard(
        ...
        val rowSignature = coordinator.rowsSnapshot().joinToString("|") { it.key.toString() }

        pendingEntries += Entry.GroupedCard("$prefix#$rowSignature", coordinator, selectableGroup)
