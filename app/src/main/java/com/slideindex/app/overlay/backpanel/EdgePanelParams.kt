...
/** Ported from AOSP SystemUI `EdgePanelParams` (Apache-2.0). */
data class EdgePanelParams(private var resources: Resources) {
    ...
    private fun getDimen(id: Int): Float = resources.getDimension(id)

    private fun getDimenFloat(id: Int): Float = TypedValue().run {
        resources.getValue(id, this, true)
        float
    }

    private fun getPx(id: Int): Int = resources.getDimensionPixelSize(id)

    fun setStaticTriggerThreshold(px: Float) {
        staticTriggerThreshold = px
    }
    ...
    fun update(resources: Resources) {
        ...
        panelWidth = getPx(R.dimen.navigation_edge_panel_width)
        panelHeight = getPx(R.dimen.navigation_edge_panel_height)
