...
fun applyMessageThemeBackground(view: View, theme: MessageThemeSpec, opacity: Float = 1f) {
    val drawable = ResourcesCompat.getDrawable(view.context.resources, theme.backgroundResId, null)?.mutate() ?: run {
        view.setBackgroundResource(theme.backgroundResId)
        view.alpha = theme.overlayAlpha(opacity)
        return
    }
    theme.backgroundTintArgb?.let { DrawableCompat.setTint(drawable, it) }
